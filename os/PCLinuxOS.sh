...
#####################################################
# uninstall statement here. not yet
#################################################
...
MyOSSettingValueLauncher="PCLinux"
MyOSSettingValueMachine="PCLinux"
MyOSSettingValueHosts="PCLinux"
MyOSSettingValueSquid="PCLinux"
MyOSSettingValuePrivoxy="PCLinux"
MyOSSettingValueTor="PCLinux"
MyOSSettingValueShutdown="PCLinux"
MyOSSettingValueOpenvpn="PCLinux"
...
case "$MyOSSettingValueMachine" in
	...
	PCLinux)
		PCLinuxIPForwardSetting=`$SysctlCommand -a > /etc/sysctl.conf.bak;$SysctlCommand -w net.ipv4.ip_forward=1; $SysctlCommand -p`
		echo $PCLinuxIPForwardSetting
...
case "$MyOSSettingValueHosts" in
	...
	PCLinux)
		SetHostPCLinux=`cp /etc/hosts /etc/hosts.bak`
		echo $SetHostPCLinux
...
case "$MyOSSettingValueSquid" in
	...
	PCLinux)
		...
		echo "acl LAN src $MyIPLanPCLinux/24" >> /etc/squid/squid.conf
		...
		echo "Your Port:"
		read MyPortAdrPCLinux
		echo "http_port $MyPortAdrPCLinux" >> /etc/squid/squid.conf
		...
		HostnamePCLinux=`uname -n`
		echo "visible_hostname $HostnamePCLinux" >> /etc/squid/squid.conf
		echo "Pluto Internet Privacy using IP: "$MyIPLanPCLinux" Port: "$MyPortAdrPCLinux""  >> /var/log/earth.log
		echo "Pluto Internet Privacy using IP: "$MyIPLanPCLinux" Port: "$MyPortAdrPCLinux""  >> /var/log/ipport.txt
...
case "$MyOSSettingValuePrivoxy" in
	...
	PCLinux)
		#Privoxy 1
		PCLinuxRemovePrivoxyConfig=`rm -rf /etc/privoxy/config`
		echo $PCLinuxRemovePrivoxyConfig
...
case "$MyOSSettingValueTor" in
	PCLinux)
		PCLinuxSettingTor=`rm -rf /etc/tor/torrc;mkdir /var/lib/tor2;mkdir /var/lib/tor3;mkdir /var/lib/tor4;mkdir /var/lib/tor5;mkdir /var/lib/tor6;mkdir /var/lib/tor7;mkdir /var/lib/tor8`
		echo $PCLinuxSettingTor
		...
		PCLinuxChangeTorAccess=`chmod 755 /var/lib/tor;chmod 755 /var/lib/tor2;chmod 755 /var/lib/tor3;chmod 755 /var/lib/tor4;chmod 755 /var/lib/tor5;chmod 755 /var/lib/tor6;chmod 755 /var/lib/tor7;chmod 755 /var/lib/tor8`
		echo $PCLinuxChangeTorAccess
...
case "$MyOSSettingValueOpenvpn" in
	PCLinux)
		SettingovpnPCLinux=`mv ovpn /etc/EarthPlanet`
		echo $SettingovpnPCLinux
