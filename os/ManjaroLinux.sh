...
ComplicatedSysctl=`ls -al /usr/bin |grep -w sysctl |awk '{print $9 }' |wc -l`
	...
	1)
		...
		case "$VarEarthPlanetStatusLinux" in
			U) ######################### Uninstall Statement ######################################
				...
				VarLinuxCmdULinux=`pacman -Sy;pacman -R openvpn squid privoxy tor --noconfirm`
...
ManjaroLinuxInstallSoftwares=`pacman -Sy;pacman -S openvpn squid privoxy tor psmisc mlocate gawk --noconfirm`
...
case "$MyOSSettingValueLauncher" in
	...
	ManjaroLinux)
		...
		ComplicatedSquid=`ls -al /usr/bin |grep -w squid |awk '{print $9 }' |wc -l`
		case "$ComplicatedSquid" in
			1)
				SquidCommand="/usr/bin/squid"
			;;
		...
		ComplicatedPrivoxy=`ls -al /usr/bin |grep -w privoxy |awk '{print $9 }' |wc -l`
		case "$ComplicatedPrivoxy" in
			1)
				PrivoxyCommand="/usr/bin/privoxy"
			;;
...
case "$MyOSSettingValueTor" in
	...
	ManjaroLinux)
		ManjaroLinuxSettingTor=`rm -rf /etc/tor/torrc;rm -rf /var/lib/tor;mkdir /var/lib/tor;chmod 755 /var/lib/tor;mkdir /var/lib/tor2;mkdir /var/lib/tor3;mkdir /var/lib/tor4;mkdir /var/lib/tor5;mkdir /var/lib/tor6;mkdir /var/lib/tor7;mkdir /var/lib/tor8`
...
case "$MyOSSettingValueShutdown" in
	ManjaroLinux)
		...
		ComplicatedPoweroff=`ls -al /usr/bin |grep -w poweroff |awk '{print $9 }' |wc -l`
		case "$ComplicatedPoweroff" in
			1)
				PoweroffCommand="/usr/bin/poweroff"
			;;
		...
		ComplicatedShutdown=`ls -al /usr/bin |grep -w shutdown |awk '{print $9 }' |wc -l`
		case "$ComplicatedPoweroff" in
			1)
				ShutdownCommand="/usr/bin/shutdown"
			;;
		...
		ComplicatedReboot=`ls -al /usr/bin |grep -w reboot |awk '{print $9 }' |wc -l`
		...
		echo "$PoweroffCommand;$ShutdownCommand 1;$PoweroffCommand -f;$ShutdownCommand now" >> /usr/local/bin/EarthPlanet/Shutdown
...
################# creating /usr/local/bin/EarthPlanet/GoHome ###########################
